...
Friend Class PDP
    ...
    Private Sub Command32_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Command32.Click
        Dim rstCred As MainDS.TblCreditoRow
        ...
        If (TPDPNumero.Text <> "") And (TPDPCredito.Text <> "") And (TPDPMonto.Text <> "") Then
            ...
            Else
                ...
                If (existe = 0) Then
                    If MainDSO.TblCredito.Rows.Contains({codi, TPDPCredito.Text}) Then
                        rstCred = MainDSO.TblCredito.Rows.Find({codi, TPDPCredito.Text})
                        'Se actualizan los datos en la tabla de creditos
                        pdpsaldo = rstCred.deuda - CDbl(TPDPMonto.Text)
                        If (pdpsaldo < 0) Then
                        ...
                        Else
                            If (pdpsaldo = 0) Then
                                ...
                                rstCred.deuda = 0
                                'rstCred("Estatus") = 4
                                MainDSO.TblSocios.FindByCI(rstCred.VCI).Deuda = 0
                            Else
                                rstCred.deuda = pdpsaldo
                                MainDSO.TblSocios.FindByCI(rstCred.VCI).Deuda = pdpsaldo
                            End If
                        End If
                        'Registra la Operacion en el Libro IE
                        rstentrar = MainDSO.TblLibroIE.NewRow
                        rstentrar.IdBK = codi
    ...
    Private Sub TPDPCredito_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles TPDPCredito.SelectedIndexChanged
        Dim rstnombre As MainDS.TblSociosRow
        Dim cred As MainDS.TblCreditoRow
        Dim codigo As String
        codigo = codi
        cred = MainDSO.TblCredito.Rows.Find({codi, TPDPCredito.Text})
        If MainDSO.TblSocios.Rows.Contains(cred("VCI")) Then
            rstnombre = MainDSO.TblSocios.FindByCI(cred.VCI)
